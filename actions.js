  ...
  function keyPress() {
    ...
    function addHtml(button) {
        ...
        if(button.textContent=="INSTRUCTIONS") {
            ...
            modalbody.innerHTML="<div class='list-type1'><ol id='olist'><li><a>Click the blocks in ascending(or descending) order of their numbers.</a></li><li><a>Incase you click the wrong block you do not get any score .i.e GAME OVER.</a></li><li><a>You win if you click all the blocks in the right order.</a></li><li><a>The time you take for finishing the game is taken into account for score calculation</a></li><li><a>The player with the lowest time per block tops the leaderboard.</a></li><li><a>If the player plays in dynamic mode(blocks moving), player gets a multiplier of 2x(his time is divided by 2)</a></li><li><a>If the player plays in hide numbers mode(guess the order by the shade of the block), player gets a multiplier of 3x.</a></li><li><a>Top 5 lowest time scores reach the leaderboard.</a></li><li><a>In challenge mode, you can set 3 time targets-primary, secondary and tertiary beyond which time will be up!</a></li><li><a>If the player doesn't make it to the top 5, a 'Try again' option allows you to restart the game with the same difficulty modes</a></li></ol></div>";
        }
